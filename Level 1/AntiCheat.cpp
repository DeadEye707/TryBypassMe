...
// ============================================================
//  PROCESS BLACKLIST
//  Two tiers: exact exe name match and substring match.
//  Exact match uses unordered_set for O(1) lookup.
//  Substring list only contains long unambiguous strings
//  to avoid false positives on legitimate processes.
// ============================================================
...
// ============================================================
//  CRC32
//  Standard CRC32 using the Castagnoli polynomial.
//  Used for .text section integrity verification.
// ============================================================
...
// ============================================================
//  DETECT
//  Core detection handler. Sets game over flag, spawns a
//  popup on a separate thread so it does not block, then
//  terminates the process after a short delay.
// ============================================================
...
// ============================================================
//  PROCESS SCAN
//  Enumerates all running processes and checks against both
//  the exact blacklist and the substring blacklist.
// ============================================================
...
// ============================================================
//  WINDOW TITLE SCAN
//  Enumerates all top-level windows and checks titles against
//  known tool signatures. Catches renamed executables that
//  still show their original window title.
// ============================================================
...
// ============================================================
//  DEBUGGER DETECTION
//  Four independent methods so bypassing one is not enough.
//  Methods: IsDebuggerPresent, NtQueryInformationProcess
//  DebugPort, PEB NtGlobalFlag, and hardware breakpoints.
// ============================================================
...
// ============================================================
//  HP FREEZE DETECTION
//  Tracks cumulative damage dealt and healing received.
//  Expected HP = MAX_HP - totalDamage + totalHealed.
//  If actual HP exceeds expected by more than the tolerance
//  window, the value is being externally frozen or set.
// ============================================================
static bool ACCheckHpFreeze()
{
    ...
    // Tolerance of 15 accounts for thread timing jitter and
    // rapid successive hits that update the counter before hpNow is read.
    // A real freeze will push HP 20+ points above expected.
...
//  Every legitimate shot increments both g_shotsFired and
//  g_ammoConsumed. If shots and consumed drift apart by more
//  than 5, ammo is being frozen externally without going
//  through the legitimate fire path.
// ============================================================
...
// ============================================================
//  HOOK DETECTION
//  Checks first bytes of critical WinAPI and ntdll functions
//  for JMP opcodes indicating an inline hook. Also compares
//  current bytes against a snapshot taken at startup to catch
//  hooks applied after the process launched.
// ============================================================
...
// ============================================================
//  CODE INTEGRITY
//  CRC32 of the .text section is stored at startup.
//  Any byte patch, NOP, or JMP redirect inside game code will
//  change the CRC and trigger detection. Checked every 3 ticks
//  because it is more expensive than the other checks.
// ============================================================
...
// ============================================================
//  BACKGROUND THREAD
//  All checks run on a 1-second interval. Code integrity
//  runs every 3 seconds because it scans the full .text section.
// ============================================================
...
// ============================================================
//  INIT
//  Snapshots original function bytes and .text CRC32.
//  Runs startup checks synchronously before the game window
//  is created, then launches the background detection thread.
// ============================================================
void ACInit()
{
    HMODULE k32 = GetModuleHandleA("kernel32.dll");
