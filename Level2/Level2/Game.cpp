...
// Force Windows subsystem so the linker looks for WinMain instead of main.
// Required when the project Linker -> System -> SubSystem is set to Console.
...
// g_gameOver is defined in AntiCheat.cpp and extern-declared in AntiCheat.h
...
static void DealDamageToPlayer(int dmg)
{
    ...
    if (hp <= 0) {
        hp = 0;
        g_gameOver = true;
        g_highScore = std::max(g_highScore, (int)ACReadEnc(g_encScore));
    }
...
static void Update(float dt)
{
    ...
    for (auto& e : g_enemies) {
        ...
        if (e.type == 1) {
            e.shootCd -= dt;
            if (e.shootCd <= 0.f) {
                e.shootCd = std::max(randf(1.5f, 3.5f) - (ACReadEnc(g_encWave) * 0.1f), 0.6f);
                Bullet* slot = nullptr;
                for (auto& b : g_bullets) if (!b.active) { slot = &b; break; }
                if (!slot) { g_bullets.push_back({}); slot = &g_bullets.back(); }
                slot->pos = e.pos;
...
static void RenderGame()
{
...

    {
        ...
        char hpstr[16];
        sprintf_s(hpstr, "%d/%d", hp, PLAYER_MAX_HP);
    ...
    if (g_reloading) {
        ...
        char am[32]; sprintf_s(am, "AMMO %d/%d", ammo, PLAYER_MAX_AMMO);
        DrawText_(dc, am, 14, 38, ammo > 5 ? RGB(180, 220, 180) : RGB(255, 80, 80), 14, true);
    }

    // HUD: Score, wave, kill count
    char sc[64]; sprintf_s(sc, "SCORE  %d", ACReadEnc(g_encScore));
    DrawText_(dc, sc, SCREEN_W - 200, 14, RGB(255, 220, 50), 16, true);
    char wv[32]; sprintf_s(wv, "WAVE   %d", ACReadEnc(g_encWave));
    DrawText_(dc, wv, SCREEN_W - 200, 34, RGB(100, 200, 255), 15, true);
    char kl[32]; sprintf_s(kl, "KILLS  %d", ACReadEnc(g_encKills));
    ...
    if (g_paused && !g_gameOver) {
        ...
        DrawText_(dc, "TryBypassMe v2.0", SCREEN_W / 2, py2 + 68, RGB(60, 120, 60), 14, false, true);

        HPEN dp = CreatePen(PS_SOLID, 1, RGB(40, 80, 40)); SelectObject(dc, dp);
        MoveToEx(dc, px2 + 20, py2 + 92, nullptr); LineTo(dc, px2 + pw - 20, py2 + 92);
        DeleteObject(dp);

        char sc3[64]; sprintf_s(sc3, "Score   %d", ACReadEnc(g_encScore));
        char wv3[32]; sprintf_s(wv3, "Wave    %d", ACReadEnc(g_encWave));
        char kl3[32]; sprintf_s(kl3, "Kills   %d", ACReadEnc(g_encKills));
        char hp3[32]; sprintf_s(hp3, "Health  %d / %d", ACReadEnc(g_encHp), PLAYER_MAX_HP);
    ...
    if (g_gameOver) {
        ...
        DrawText_(dc, "TryBypassMe v2.0", SCREEN_W / 2, SCREEN_H / 2 - 68, RGB(100, 180, 255), 18, false, true);
        char sc2[64]; sprintf_s(sc2, "SCORE:  %d", ACReadEnc(g_encScore));
        char hi[64];  sprintf_s(hi, "BEST:   %d", g_highScore);
        char wv2[32]; sprintf_s(wv2, "WAVE:   %d", ACReadEnc(g_encWave));
        char kl2[32]; sprintf_s(kl2, "KILLS:  %d", ACReadEnc(g_encKills));
...
int WINAPI WinMain(_In_ HINSTANCE hInst, _In_opt_ HINSTANCE, _In_ LPSTR, _In_ int)
{
    ...
    g_xorKey = ((uint32_t)rand() << 16)
        ^ ((uint32_t)rand() & 0xFFFF)
        ^ (uint32_t)(GetTickCount64() & 0xFFFFFFFF)
        ^ ((uint32_t)GetCurrentProcessId() << 7);
    if (g_xorKey == 0) g_xorKey = 0xDEADF00D;
    ...
    g_hwnd = CreateWindowA("TryBypassMe", "TryBypassMe v2.0 | WASD + Mouse",
